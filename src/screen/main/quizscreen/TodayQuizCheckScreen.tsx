...
const TodayQuizCheckScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    const getIsCompleteTodayQuiz = getItemFromAsyncStorage<boolean>(IS_COMPLETE_TODAY_QUIZ);
    const getReviewQuizDataList = getItemFromAsyncStorage<any[]>(REVIEW_QUIZ_DATA_LIST);
    const getTodayQuizAnswerList = getItemFromAsyncStorage<any[]>(TODAY_QUIZ_ANSWER_LIST);
    const getTodayQuizBallState = getItemFromAsyncStorage<any[]>(TODAY_QUIZ_BALL_STATE);
